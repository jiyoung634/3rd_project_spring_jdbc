					...
					<div class="col-md-12">
						<div class="card">
							<div class="card-header">
								<strong class="card-title">과정 관리</strong>
								<button type="button"
									class="btn btn-info btn-sm btn-margin-left" data-toggle="modal"
									data-target="#addCourse">새로 등록</button>
							</div>
							<div class="card-body">
									...
									<tbody>
										<c:forEach var="bc" items="${bcList_ds}">
											<tr>
												<td>${bc.bc_id}</td>
												<td>${bc.bc_title}</td>
												<td><button type="button" class="btn btn-info btn-sm btnDeleteCourse" value="${bc.bc_id}" ${(bc.delete_status=='Y')?"":"disabled"}>삭제</button></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
			...
			<div class="modal-dialog">
				...
				<form id="new insert" action="${pageContext.request.contextPath}/manager/basicinfo/course/insert" method="POST" >
					<div class="modal-header">
						<h4 class="modal-title">기초 과정 등록</h4>
					...
					<div class="modal-body">
						...
						<div class="form-group">
							<label for="bc_title" class=" form-control-label">과정명</label> <input
								id="bc_title" name="bc_title" placeholder="과정명을 입력하세요" class="form-control" required>
						</div>
